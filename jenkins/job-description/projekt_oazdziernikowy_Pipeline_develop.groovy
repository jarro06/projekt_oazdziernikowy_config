...
def job_config = [
        job: [
                name: "projekt_oazdziernikowy_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'projekt_oazdziernikowy_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/projekt_oazdziernikowy_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = lib.de.allianz.projekt_oazdziernikowy.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "projekt_oazdziernikowy_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
